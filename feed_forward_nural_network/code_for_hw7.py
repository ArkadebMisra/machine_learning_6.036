...
class Linear(Module):
    ...
    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        ...
        self.dLdW0 = np.sum(dLdZ, axis=1).reshape((dLdZ.shape[0], 1))  # Your code
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        return dLdA*(1-self.A**2)  # Your code: return dLdZ (m, b)
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        ...
        a = np.where(Y==1,np.log(Ypred),np.log(1- Ypred))*(-1)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            #print(Ypred)
            #print(Y)
            c_loss = self.loss.forward(Ypred, Y[:,index:index+1])
            self.backward(self.loss.backward())
            self.sgd_step(lrate)
            self.print_accuracy(it, X, Y, c_loss, every=250)                                  
...
# disp.classify(X, Y, nn, it=100000, lr=0.005)
